...
export default function Lineage() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      .catch(() => setError('血缘资产暂时不可用，请确认 medical-data-governance 已生成 lineage 产物并挂载到后端。'))
      ...
      series: [
        {
          ...
          data: nodeNames.map((name) => ({
            ...
            itemStyle: { color: name.startsWith('ads.') ? '#c41d7f' : name.startsWith('dws.') ? '#722ed1' : name.startsWith('dwd.') ? '#1d39c4' : '#1677ff' },
          })),
  ...
  if (error) {
    return <Alert type="error" showIcon message="血缘加载失败" description={error} />;
  }
  ...
  return (
    <div>
      ...
      <Row gutter={16} style={{ marginBottom: 24 }}>
        <Col span={6}>
          <KpiCard title="表级血缘" value={summary?.table_edge_count || 0} unit="条" />
        </Col>
        <Col span={6}>
          <KpiCard title="字段级血缘" value={summary?.column_edge_count || 0} unit="条" />
        </Col>
        <Col span={6}>
          <KpiCard title="表目录" value={summary?.table_count || 0} unit="张" />
        </Col>
        <Col span={6}>
          <KpiCard title="主题域" value={summary?.subjects.length || 0} unit="个" />
        </Col>
      </Row>
          ...
          <Card title="表目录">
            <Table
              ...
              columns={[
                ...
                { title: '层级', dataIndex: 'layer', width: 72, render: (value) => <Tag color={layerColor(value)}>{value}</Tag> },
      ...
      <Card title="字段级血缘" style={{ marginTop: 16 }}>
        <Table
          ...
          columns={[
            { title: '目标字段', render: (_, row) => `${row.target_table}.${row.target_column}`, ellipsis: true },
            { title: '来源字段', render: (_, row) => `${row.source_table}.${row.source_column}`, ellipsis: true },
